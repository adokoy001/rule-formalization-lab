...
Rule Formalization Labは、規約や法令など自然言語で書かれたルールを、人が確認できる有限の形式モデルへ写し、ルールの衝突や抜けに気づきやすくするための趣味の研究開発プロジェクトです。

作成: 2026-09-14 / 更新: 2026-09-16

## 何を目標にしているか

原文の版、採用した解釈、仮定、検査範囲を明示したうえで、**「どの条件の組合せで問題が起きるか」を具体例として示し、同じ入力から結果を再現・再検査できるツール**を目指しています。

最初は小さな架空規約を扱い、長期的には刑法や刑事訴訟法の限定した論点・条文群へ段階的に広げます。結果は法令そのものに対する法的判断ではなく、採用した解釈と有限範囲の中で再確認すべき箇所を示すものです。

## 何ができるものを目指しているか

- 同じ状況から両立しない結論が出る条件を、具体的な入力例と関係する規則付きで示す。
- 結論が必要だと指定した照会について、どの条件で結論が抜けるかを示す。
- 一度も適用されない規則や、例外によって常に抑止される規則を見つける。
- 義務と禁止を同時に守れる行動があるか、明示的な許可を実際に行使できるかを調べる。
- 改定前後や複数の解釈を比べ、どの状況で結論が変わるかを示す。
- 結果を原文、解釈、仮定、対象範囲へたどり、保存した証拠を別の検査器で再確認する。
- 将来はLLMに形式化候補を作らせ、人のレビューを通った部分だけを検証へ渡す。

## どのように実現するか

```mermaid
flowchart LR
    S["版を固定した原文"] --> C["解釈候補<br>現在は手書き／将来はLLM支援"]
    C --> R["候補とは別に管理するレビュー・仮定・検査範囲"]
    R --> I["決定的な変換"]
    I --> M["有限形式モデル"]
    M --> K["自前の探索カーネル"]
    K --> E["具体例と検査証拠"]
...
1. 取得した原文そのもの（raw bytes）、版、抽出位置をhash付きpackageとして固定します。
2. 解釈候補を原文、レビュー、仮定、検査範囲とは別の入力として保存します。
3. 対応している意味だけを決定的な変換で有限モデルへ移します。未解決事項や未対応の表現は、推測で補わず未確定として残します。
4. 真偽値、明示された有限候補、範囲付き整数や有限の行動候補を列挙し、決定の衝突、結論の抜け、到達可能性、規範の履行可能性などを調べます。
5. カーネルが作った全件証拠を、探索カーネル本体の処理を流用しない別実装の検査器で再構成します。打切り、入力不正、未対応、証拠不一致は「問題なし」として扱いません。

現在の検証カーネルはPython標準ライブラリだけで動き、本番の推論・探索を既成のSAT/SMTなどの外部推論エンジンへ委譲しません。規模を広げる段階では、同じ意味を保った自前SAT探索と、変換・証拠を検査する経路を追加する計画です。詳しい全体設計は[開発計画](planning/README.md)にあります。
...
> **用途上の注意:** 趣味・研究用の検証支援プロジェクトであり、法律相談や現実の法的判断を提供しません。結果は、明示した有限モデル、原文版、解釈、仮定の範囲に限られます。

本プロジェクトが作成したコード、文書、架空fixtureは[MIT License](LICENSE)で提供します。第三者由来データには別の条件が適用されるため、[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)も参照してください。
