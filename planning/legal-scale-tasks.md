...
T11以降も一度で全部作る指示ではない。仕様・最小fixture・実装・独立checker・検証に小分けして、都度日誌を残す。
...
別profile`finite-procedure-time/1`と`procedurekernel`を実装した。架空の申請、受領、審査、決定、通知を5 slot・5規範・16 contextで表し、`(tick, phase)`、排他的観測終了、包含期限、strict/non-strict順序、部分禁止、起算点欠落・時刻未確定を固定した。

producer非依存checkerが全trace、規範別status、completion feasibility、observed outcome、集計を再構成する。専用31件と全体回帰が合格し、保存certificateは外部anchor付き`VERIFIED`となる。[仕様](../docs/procedure-time-kernel-v0.1.md)と[検証記録](../docs/verification-t09-2026-09-16.md)を参照する。連続時間、一般暦、反復event、open-world観測、権限・裁量は未対応である。

## T10: 刑事訴訟法の最小pack（限定profile完了）

e-Govの現行施行revisionから55条・203条から206条を5 source unitとして保存し、203条から205条経路の標準数値期限だけを13 contextへ手書きした。203条48時間、205条の受領後24時間と拘束後72時間を別normにし、勾留請求又は公訴提起をany-of、送致手続と受領を別eventとした。

task、candidate、`manual_fixture_review`、coverage全5件、model、certificate、全文quoteをbindingへ結び、producer非依存checkerが`MANUAL_MODEL_BINDING_VERIFIED`を返す。206条事情説明は数値超過を自動で消さず、釈放規定triggerと実際の釈放観測を分離した。専用25件と全358件が合格した。[pack](../examples/procedures/t10-criminal-procedure-203-205/README.md)、[法源確認](../research/2026-09-16-criminal-procedure-t10-source-review.md)、[検証記録](../docs/verification-t10-2026-09-16.md)を参照する。

完了は標準数値期限と暫定的手作業bindingに限定する。204条経路、一般暦、一般延長、206条の評価、意味的lowering、法律専門家レビュー、適法・違法は未対応である。
