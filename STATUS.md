...
- 決定の基礎検査向けcompact証拠`finite-decisions-compact-jsonl/1`。整数定数境界から重み付きセルを作り、strict JSON Linesを逐次生成・検査する。Club20は問題版416状況を80セル、修正版を64セルへ圧縮して、全列挙と同じ件数・最初の具体例を返す。
- compact checkerはproducerと境界plannerをimportせず、partition、意味、集計、全範囲commitmentを独自に再構成する。途中切断、欠落、重複、並替え、非canonical JSON、外部hash不一致を拒否する。
...
- T12基礎実装反映後の全suite 397件と`compileall`が合格。従来の決定・原文・解釈・規範・手続・ベンチマークに加え、境界分割、compact逐次証拠、容量境界、想定外例外、改ざんを回帰検査する。
...
- norm/procedure/T10 bindingの公開CLIは捕捉可能な想定外`Exception`を内容非表示の`INTERNAL_ERROR`、未確定、exit 2にする。norm/procedureの新規証拠は公開後の失敗時も自分が作ったinodeだけを巻き戻す。
...
- procedure producer/checkerはtraceとcaseの正規JSON bytesを追加前に増分検査する。保存形式とhashは維持したが、受理済み証拠のメモリ保持と入力certificateの初期serializeは残る。
...
[使い方](README.md)、[compact証拠仕様](docs/compact-certificate-v0.1.md)、[T12検証記録](docs/verification-t12-2026-09-16.md)、[有限手続・時間仕様](docs/procedure-time-kernel-v0.1.md)、[T09検証記録](docs/verification-t09-2026-09-16.md)、[T10検証記録](docs/verification-t10-2026-09-16.md)、[T10法源確認](research/2026-09-16-criminal-procedure-t10-source-review.md)、従来の各仕様・検証記録、[本日の日誌](devlog/2026-09-16.md)から確認できる。
...
- [T12の容量対策](planning/t12-evidence-scaling.md)は決定の基礎検査について実装した。未実装なのはdiff・到達可能性・段階別到達可能性のcompact化、規範・手続証拠のJSON Lines化、全積上限を越える探索、自前SAT/DPLL、歴史的T06 source bindingを移行したうえでの旧`rulekernel` CLI例外契約更新である。
...
compact経路でも全積10,000と出力8 MiBを維持する。整数定数境界で結果が一定になる場合に証拠行数と保存量を減らすが、全 concrete case commitmentの再計算は行うため、探索時間を省く一般的な高速化ではない。整数変数同士の比較では全整数軸をsingletonへ戻し、Boolean中心のT06静的matrixには圧縮効果がない。
...
T12は容量問題に対する基礎sliceまで実装した。次に容量対策を広げる場合は、実測で必要になった経路からcompact形式を追加し、旧形式を置き換えず対照検査を残す。
